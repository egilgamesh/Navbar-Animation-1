...
public partial class TabBarView : ContentView
{
	...
	double defaultIconTranslation =>
		((backGraphicsView.Height - innerRadius - iconHeight) / 2) + innerRadius;
	...
	public TabBarView()
	{
		...
		InitializeComponent();
		iconViews = buttonsGrid.Children.Cast<ContentButton>().Select(cb => cb.Content).
			Cast<TabBarIconView>().ToList();
	...
	private void ButtonTapped(object sender, EventArgs e)
	{
		...
		var oldIconAnimation = new Animation(v => { oldIconView.TranslationY = v; },
			oldIconView.TranslationY, defaultIconTranslation, easing: Easing.SpringOut);
		var newIconAnimation = new Animation(v => { iconView.TranslationY = v; },
