...
public class TabBarViewDrawable : IDrawable
{
	...
	public TabBarViewDrawable(float innerRadius, float outerRadius, Color barFillColor,
	...
	private PathF CreatePath(RectF dirtyRect)
	{
		...
		float c = (float)((Math.Pow(CircleCenter.X, 2) + Math.Pow(CircleCenter.Y, 2) -
			Math.Pow(outerRadius, 2)));
		float d = (float)((Math.Pow(2 * CircleCenter.X, 2)) -
			(4 * (Math.Pow(y46, 2) - (2 * CircleCenter.Y * y46) + c)));
		...
		float alpha = (float)((Math.PI / 2) -
			Math.Atan((CircleCenter.X - p4.X) / (p4.Y - CircleCenter.Y)));
		...
		#endregion
		#region p3 p9
		float scale = (float)((outerRadius - innerRadius) /
			Math.Sqrt(Math.Pow(p4.X - p2.X, 2) + Math.Pow(p4.Y - p2.Y, 2)));
